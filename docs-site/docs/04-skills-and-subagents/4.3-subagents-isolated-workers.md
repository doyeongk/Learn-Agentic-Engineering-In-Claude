...
The `/project:init` command used subagents at several points:
...
1. What happens to the context when a subagent finishes its task?
   ...
   - B) Only a condensed summary is returned to the parent
   - C) The context is saved to a file for later
   - D) The context is shared with other running subagents
