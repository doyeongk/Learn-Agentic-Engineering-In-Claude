...
Every module has a command launcher in `.claude/commands/`. When you run `/3-1-what-is-context`, the launcher file tells Claude Code:
...
### Layer 4: The Generator — /init

The `/init` command is the most complex orchestrator. It:
...
```mermaid
graph TD
    U["User runs /start"] --> CMD["Command reads progress.json"]
    ...
    INIT["/init"] --> R1["Subagent: Research"]
...
2. In your own words, trace what happens from the moment you run `/start` to the moment a module is delivered. Name each component involved and its role.
