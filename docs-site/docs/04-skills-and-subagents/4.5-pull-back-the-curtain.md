...
**"This is too complex for a learning tool."**
Count the actual components: one CLAUDE.md, four skills, 18 command launchers, 18 content files, one progress file, one landing page. Each is a simple text file. The complexity comes from how they compose, not from any individual piece.

**"The AI understands all of this naturally."**
It does not. Without CLAUDE.md, Claude Code would not know this is a learning platform. Without skills, it would not know how to teach. Without command launchers, it would not know the sequence. Every capability is explicitly defined in context files. The AI's role is to follow those instructions well.

**"You need to understand all of this to use the system."**
You do not. A learner can simply run `/project:start` and follow the modules without knowing anything about the architecture. Understanding the architecture is a bonus — it is the equivalent of understanding how a car engine works while driving. Useful, but not required.
