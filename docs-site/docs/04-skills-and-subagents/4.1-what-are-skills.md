...
Skills are not loaded automatically. They are loaded by commands — the slash command files in `.claude/commands/`. When you run a module command (e.g., `/3-1-what-is-context`), the command file tells Claude Code to read the training-delivery skill before delivering the module.

This is the loading chain:

```mermaid
graph LR
    A["/command"] --> B["Command file reads SKILL.md"]
