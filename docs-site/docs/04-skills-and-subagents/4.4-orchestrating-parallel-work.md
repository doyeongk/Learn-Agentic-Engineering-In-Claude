...
> **What you will build**: A `/daily-digest` command that launches parallel subagents to gather information, then formats the results using a custom skill.
>
...
```
Describe what just happened when I ran /daily-digest. What files were read, what subagents were launched, and how were the results combined?
```
...
This pattern scales to much more complex workflows. The `/init` command that generated this curriculum uses the same architecture — commands, skills, and subagents working together.
